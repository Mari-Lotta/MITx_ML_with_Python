...
def epsilon_greedy(state_vector, epsilon):
    ...
    if np.random.rand() < epsilon:
        # Explore: choose a random action-object pair
        action_index = np.random.randint(0, NUM_ACTIONS)
        object_index = np.random.randint(0, NUM_OBJECTS)
    else:
        # Exploit: choose the best action-object pair according to the Q-function
        q_values_action, q_values_object = model(state_vector)
        action_index = torch.argmax(q_values_action).item()
        object_index = torch.argmax(q_values_object).item()
...
def deep_q_learning(current_state_vector, action_index, object_index, reward,
    ...
    Returns:
        None
    """
    # Compute Q(s', c') = max_a Q(s', a) for the next state
    with torch.no_grad():
        q_values_action_next, q_values_object_next = model(next_state_vector)
    maxq_next = 0.5 * (q_values_action_next.max().item() + q_values_object_next.max().item())

    # Get the current Q-value Q(s, c)
    q_values_action_cur, q_values_object_cur = model(current_state_vector)
    q_value_cur = 0.5 * (q_values_action_cur[action_index] + q_values_object_cur[object_index])

    # Compute the target Q-value
    target_q_value = torch.tensor(reward, dtype=torch.float32) if terminal else torch.tensor(reward + GAMMA * maxq_next, dtype=torch.float32)

    # Compute the loss
    loss = F.mse_loss(q_value_cur, target_q_value)
...
def run_episode(for_training):
    ...
    while not terminal:
        ...
        if for_training:
            # Update Q-function using deep Q-learning
...
if __name__ == '__main__':
    ...
    axis.set_title(('DQN: nRuns=%d, Epsilon=%.2f, Epi=%d, alpha=%.4f' %
